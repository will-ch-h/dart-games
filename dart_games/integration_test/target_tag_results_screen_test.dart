...
void main() {
  ...
  // Helper function to get mock API
  ...
  // Helper function to set shield max value by programmatically calling the slider's onChanged callback
  // Shield max range: 1-10, divisions: 9
  Future<void> setShieldMax(WidgetTester tester, int shieldMax) async {
    final sliderFinder = find.byType(Slider);
    expect(sliderFinder, findsWidgets); // May have multiple sliders (shield max, possibly others)

    // Find the Shield Max slider specifically
    // Look for slider near "Shield Max:" text
  ...
  // Helper function to quickly complete a game and reach results screen
  ...
  Future<void> completeGameToVictory(WidgetTester tester, String player1Name, String player2Name) async {
    ...
    await throwDart(tester, target1, multiplier: 'triple'); // 3 shields - TAGGED IN!
    await tester.pump(const Duration(milliseconds: 300));
    ...
    await throwDart(tester, target2, multiplier: 'single'); // Shield 1
    await tester.pump(const Duration(milliseconds: 300));
    await throwDart(tester, target2, multiplier: 'single'); // Shield 2
    await tester.pump(const Duration(milliseconds: 300));
    await throwDart(tester, 1, multiplier: 'single'); // Miss
    await tester.pump(const Duration(milliseconds: 300));
    await removeDarts(tester);
  ...
  // Helper function for team mode - teams share targets
  // Based on Test 8 from target_tag_gameplay_test.dart
  // Uses provider to get team info and attack until elimination
  Future<void> completeGameToVictoryTeamMode(WidgetTester tester, String team1Player, String team2Player) async {
    ...
    await throwDart(tester, teamTargetNum, multiplier: 'triple'); // 3 shields - TAGGED IN!
    await tester.pump(const Duration(milliseconds: 500));
    await tester.pump();
    await throwDart(tester, 1, multiplier: 'single'); // Miss
    await tester.pump(const Duration(milliseconds: 500));
  ...
  group('Target Tag - Results Screen Tests', () {
    ...
    testWidgets('Test 2: Play Again - Settings Preservation Solo Mode', (WidgetTester tester) async {
      ...
      await addPlayer(tester, 'Player2');
    ...
    testWidgets('Test 3: Change Settings - Return to Menu with Preserved Settings', (WidgetTester tester) async {
      ...
      final changeSettingsButton = find.text('Change Settings');
    ...
    testWidgets('Test 5: Play Again - Team Mode Settings and Team Assignment Preserved', (WidgetTester tester) async {
      ...
      await addPlayer(tester, 'TeamA1');
      await addPlayer(tester, 'TeamA2');
      await addPlayer(tester, 'TeamB1');
      await addPlayer(tester, 'TeamB2');
    ...
    testWidgets('Test 6: Play Again - Hero Bonus Setting Preserved', (WidgetTester tester) async {
      ...
      await addPlayer(tester, 'Hero1');
      await addPlayer(tester, 'Hero2');
