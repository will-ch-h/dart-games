...
├── integration_test/                # UI automation tests (373 tests)
└── assets/
...
# Optional: Run UI automation tests (373 tests, requires chromedriver)
...
**UI Automation Test Coverage (373 tests, one-test-per-process architecture):**
- Each test runs in its own `flutter drive` process for full isolate-level isolation
- **Sequential runner** (`run_ui_tests.bat`): One game at a time, ~519 minutes (~8h 39m). Interactive Chrome sessions visible. Best for debugging.
- **Parallel runner** (`run_ui_tests_parallel.bat`): All 5 games simultaneously, ~145 minutes (~2h 25m, ~3.5x faster). Fully headless — no visible Chrome sessions. Each game gets its own ChromeDriver (ports 4444-4448) and backend server (ports 9001-9005). Requires 16GB+ RAM.
- Per-session database isolation (`X-DB-Session` header) prevents cross-test data pollution
- Target Tag (71 tests): Menu settings, gameplay mechanics, visual validation, add player, results screen, save/resume
- Carnival Derby (42 tests): Menu, gameplay, bust mechanics, skip turn, edit score, results screen, save/resume
- Monster Mash (68 tests): Add player, menu settings, gameplay, buff effects, speed play, edit score, results screen, visual validation, save/resume
- Reef Royale (84 tests): Add player, menu settings, gameplay, coral claiming, edit score, results screen, visual validation, showcase, screenshot, save/resume
- Clockwork Quest (108 tests): Add player, menu settings, gameplay, gear progression, edit score, results screen, save/resume, screenshot
